"""
X3D Temporal Shift RGB Backbone for EPAM-Net
...
class BlockX3D(nn.Module):
    """
    BlockX3D 3D building block for X3D with Temporal Shift Module
    ...
    Args:
        ...
        se_ratio (float | None): SE reduction ratio. If None, no SE module. Default: None
        use_swish (bool): Whether to use swish activation. Default: True
        use_sta (bool): Whether to use spatial-temporal attention. Default: False
    ...
    def temporal_shift(self, x, fold_div=8, inplace=False):
        """
        Temporal Shift operation

        Args:
            x (torch.Tensor): Input tensor of shape (N, C, T, H, W)
            fold_div (int): Fold division factor. Default: 8
...
class X3DTemporalShift(nn.Module):
    """
    X3D Temporal Shift Backbone for RGB stream

    This is an efficient 3D CNN architecture that uses Temporal Shift Module
    to exchange information across temporal dimension without 3D convolution
    in time axis, significantly reducing computational cost.
    ...
    Args:
        ...
        stage_blocks (tuple | None): Number of blocks per stage. Default: None
        spatial_strides (tuple): Spatial strides of residual blocks. Default: (2, 2, 2, 2)
        se_style (str): SE module style ('half' or 'all'). Default: 'half'
        se_ratio (float | None): SE reduction ratio. Default: 1/16
        use_swish (bool): Whether to use swish activation. Default: True

    Input Shape:
        - (N, 3, T, H, W) where T=16, H=W=224 (typical)

    Output Shape:
        - (N, feat_dim, T', H', W') where T'=16, H'=W'=7 (typical)
    ...
    def __init__(self,
        ...
        print('RGB backbone stage_blocks:', self.stage_blocks)
    ...
    def forward(self, x):
        ...
        Args:
            x (torch.Tensor): Input RGB video tensor of shape (N, 3, T, H, W)
...
if __name__ == '__main__':
    # Test RGB backbone
    model = X3DTemporalShift(gamma_w=1, gamma_b=2.25, gamma_d=2.2, use_sta=False, se_style='half')
    model.init_weights()

    # Typical input: batch=2, channels=3, frames=16, height=width=224
    input_tensor = torch.rand(2, 3, 16, 224, 224)
    output = model(input_tensor)
    print(f"RGB Backbone output shape: {output.shape}")
    print(f"Output feature dimension: {model.feat_dim}")
    # Expected output: (2, 432, 16, 7, 7) for default config
