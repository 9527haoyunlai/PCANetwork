...
head_cfg = dict(
    ...
    loss_weights=[1.0, 1.0, 0.5, 0.8],  # ← 降低Pose权重：1.5→1.0，避免Pose崩溃
    average_clips='prob')
...
train_pipeline = [
    ...
    dict(type='RandomResizedCrop', area_range=(0.56, 1.0)),  # ← 保守策略：0.40→0.56
    dict(type='Resize', scale=(224, 224), keep_ratio=False),
    dict(type='Flip', flip_ratio=0.5, left_kp=left_kp, right_kp=right_kp),
    # dict(type='ColorJitter', brightness=0.4, contrast=0.4, saturation=0.4, hue=0.15),  # ← 暂时禁用
    dict(
...
# ==========================================
# 测试Pipeline（TTA：10 clips）
# ==========================================
test_pipeline = [
    dict(
        type='MMUniformSampleFrames',
        clip_len=dict(RGB=8, Pose=32),
        num_clips=3,  # ← 降低TTA：10→3，减少过拟合
        test_mode=True),
    dict(type='MMDecode'),
    dict(type='MMCompact', hw_ratio=1., allow_imgpad=True),
    dict(type='Resize', scale=(256, 256), keep_ratio=False),
...
train_cfg = dict(
    ...
    max_epochs=50,  # ← 从Epoch 11继续训练到Epoch 50
    val_begin=1, 
    val_interval=1)
...
optim_wrapper = dict(
    optimizer=dict(
        type='SGD', 
        lr=0.005,             # ← 降低学习率：0.008→0.005
        momentum=0.9, 
        weight_decay=0.0005), # ← 增加正则化：0.0003→0.0005
    clip_grad=dict(max_norm=20, norm_type=2))  # ← 降低梯度裁剪：40→20

# 学习率调度策略（保守策略：无warmup，直接余弦退火）
param_scheduler = [
    dict(
...
# ← 从Epoch 11最佳checkpoint继续训练（57.45%）
load_from = 'work_dirs/pcan_ntu60_95target/best_acc_RGBPose_1:1_top1_epoch_11.pth'
resume = False  # ← 只加载权重，重置scheduler
...
default_hooks = dict(
    ...
    checkpoint=dict(
        ...
        rule='greater',
        max_keep_ckpts=10),      # ← 保留10个最佳checkpoint
